...
const string intro = "\n"
    "April tags detection on infant forehead\n"
    "Pediatric Perimeter\n"
    "\n";
...
public:
  ...
  Demo() :
    ...
    m_draw(true),
    m_arduino(false),
    m_timing(true), //was initially false - Dpn
  ...
  void setup() {
    ...
    void setupVideoFeed() { //a copy of the setupVideo function of the original clone, for Camera feed
  ...
  void setupVideo(){
   ...
   // if (m_brightness >= 0) {
    ...
    m_cap = cv::VideoCapture("capture1.avi");
  ...
  void print_detection(AprilTags::TagDetection& detection) const {
    //cout << "Print Detection called.  Id: " << detection.id
  ...
  //Now, check if the rotation angles are in vicinity of required angles. - Dpn
    //writing to file for python script? - Dpn
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    image_gray = image_gray*4; //How to scale to maximize detection? - Dpn
    cv::threshold(image_gray,image_gray, 60,255,CV_THRESH_TOZERO); //How to choose threshold to maximize detection? - Dpn
    ...
    if(detections.size() == 0) //printing zeros for python script analysis
    {
      cout << detections.size() << "," <<detections.size() << "," <<detections.size() << "," <<detections.size()
           << "," <<detections.size() << "," <<detections.size() << "," <<detections.size() << endl;
    }
    for (int i=0; i<detections.size(); i++) {
      print_detection(detections[i]);
    }
    if (m_draw) { // show the current image including any detections
      for (int i=0; i<detections.size(); i++) { 
        detections[i].draw(image); // also highlight in the image
      }
    ...
    if (m_arduino) {
      if (detections.size() > 0) {  // only the first detected tag is sent out, as Pedi Peri only has 1 tag. - Dpn
        Eigen::Vector3d translation;
        Eigen::Matrix3d rotation;
  ...
  void loop() {
    ...
    double last_t = tic();
    while (true) {
      m_cap >> image; //capture frame
      processImage(image, image_gray);
      video2<<image_gray;
      video1<<image;
      if (++frame % 30 == 0) { // print out the processing frame rate
        double t = tic();
        cout << "  " << 30./(t-last_t) << " fps" << endl;
        last_t = t;
      }
...
int main(int argc, char* argv[]) {
  Demo demo;
  demo.parseOptions(argc, argv); // process command line options
  demo.setup();
  if (demo.isVideo()) {
    demo.setupVideo(); // setup image source, window for drawing, serial port...
    //demo.setupVideoFeed(); // setup image source, window for drawing, serial port...
    demo.loop(); // the actual processing loop where tags are detected and visualized
  } else {
    demo.loadImages(); // process single image
  }
  return 0;
}
